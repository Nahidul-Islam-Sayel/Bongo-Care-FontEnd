...
const[testname,setTestName]=useState("");
 ...
 {data[0]&& <div class="row gutters-sm">
     ...
     <div class="col-md-8">
     <input className='searchbar' type="text" placeholder='Name' 
          onChange={(event)=>{setSearch(event.target.value);}}
          /><br/>
              <input className='searchbar' type="text" placeholder='Test Name' 
  ...
  <tbody>
   {allUser&&allUser.filter((data)=>{
      ...
      else if(data.name?.toLowerCase().includes(search?.toLowerCase())&&data.testName?.toLowerCase().includes(testname?.toLowerCase())){
          return data;
      }
